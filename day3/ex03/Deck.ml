...
let newDeck unit : t =
  let rec shuffle a b lst =
    match lst with
    | [] -> a @ b
    | e :: t -> (
      if Random.bool () then shuffle (e::a) b t
      else shuffle a (e::b) t
    )
  in
  shuffle [] [] Card.all |> shuffle [] []
...
let drawCard = function
  | [] -> raise (Failure "Deck.drawCard: Cannot draw card from empty deck!")
  | e :: t -> (e, t)
